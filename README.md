...
ERMBG Web 通过配置中的 `services.direct_worker_url` 连接主 Direct Worker。
共享默认值写在 `ermbg.config.json`;机器相关覆盖写在 gitignored
`ermbg.local.json`。如果本地和远端 worker 同时存在,把它们写入
`services.direct_worker_urls`,Web 后端列表会显示
`direct-worker:<name>` / `direct-corridorkey:<name>` 和实际 URL。环境变量
`ERMBG_DIRECT_URL` 可用于临时覆盖主地址。
...
```bash
# sync local source to the remote source tree, then restart the remote worker
