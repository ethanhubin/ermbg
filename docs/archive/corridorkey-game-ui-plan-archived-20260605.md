# 游戏 UI / CorridorKey 专题

本文是游戏 UI、CorridorKey profile、ShadowPatch 和批量验证的专题细节文档。
当前主线流程以 `docs/architecture.md` 为准;本文不单独定义生产入口。

这是游戏 UI 素材的专题说明。主线已转向由 ERMBG 拥有 routing、并按 profile
做执行: ERMBG 决定最终路径和参数,然后由 Direct Worker 运行 PyMatting Known-B、
CorridorKey、passthrough 或 PyMatting fallback。

## 概要

- 游戏 UI 素材先经过 ERMBG strategy 路由,并在 matting 前产出最终的
  `execution_profile`。
- 从活跃 route 中移除旧的 AutoMatte 全程 matting 后端。
- 未知背景路由到 PyMatting fallback;auto 不再调用 RMBG。
- route 分析、参数选择、CorridorKey/PyMatting 执行、ShadowPatch 和元数据生成,
  都在 Direct Worker 内完成。
...
结果: 83/83 全部成功完成。这早于最终的 85 样本 auto-route 契约,但作为
...
结果: 85/85 全部通过 Web/API `backend=auto` 成功完成。活跃集为 56 个按钮、
20 个 icon/effect 样本和 9 个角色样本。route 分布为 37 个 PyMatting Known-B
case 和 48 个 CorridorKey case。
...
`ermbg.corridorkey_runner.LocalCorridorKeyClient`,由 Direct Worker 使用。
