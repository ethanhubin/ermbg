...
## Known-B Candidate Trimap

Analyze may attach a selected PyMatting Known-B candidate preview asset with
`execution_role=pymatting_explicit_trimap`. Web Execute decodes that three-state
PNG (`0=sure_bg`, `128=unknown`, `255=sure_fg`) and posts it to Direct Worker as
`pymatting_explicit_trimap`.

When this upload is present, the Known-B executor consumes it directly and skips
rebuilding the base trimap. User keep/remove masks are still applied inside the
executor, with remove overriding keep. If the asset is missing or the route is
not PyMatting Known-B, execution falls back to the existing trimap builder.
